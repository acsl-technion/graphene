...
{
	return (long long)g_base_pool_ptr;
}
	...
	)
{
	...
	//debug("sizeof pci=%lld, cci=%lld, sbc=%d\n", sizeof(item_t), sizeof(crypto_item_t), SUB_PAGE_COUNT);
...
*/
	srand(time(NULL));
	for (int i=0;i<sizeof(sgx_aes_gcm_128bit_key_t);i++) {
		g_eviction_key[i] = rand() % 256;
	}
...
int try_evict_page(item_t* pce)
{
	...
	g_aptr_crypto_page_cache_l1.remove(page_index,0);
...
g_aptr_crypto_page_cache_llc.lock(page_index);
g_aptr_crypto_page_cache_l1.lock(page_index);
	item_t* it = g_aptr_crypto_page_cache_l1.get_internal(page_index);	
#ifdef APTR_RANDOM_ACCESS
	bool is_minor = it != NULL && it->is_valid[sub_page_index] > 0;
#else
	...
	if (is_minor) // minor #PF
	{
		...
		aptr->prm_ptr = it->epc;
it->is_valid[sub_page_index]++;
it->is_write[sub_page_index] |= aptr->hint_write;
//debug("got minor for page %d sub %d is_valid %d ptr %p\n", page_index, sub_page_index, it->is_valid[sub_page_index], it);
g_aptr_crypto_page_cache_l1.unlock(page_index);
g_aptr_crypto_page_cache_llc.unlock(page_index);
		return;
	}
//g_aptr_crypto_page_cache_l1.unlock(page_index);
/*
...
*/
...
#else
	if (crypto_item != NULL) // in crypto cache
#endif
	{
	...
	//debug("decrypted page %d subpage %d\n", page_index, sub_page_index);
...
//g_aptr_crypto_page_cache_l1.lock(page_index);
	item_t* found = g_aptr_crypto_page_cache_l1.try_add(page_index, prm_ptr, sub_page_index, aptr->hint_write);
#else
	item_t* found = g_aptr_crypto_page_cache_l1.try_add(page_index, prm_ptr);
#endif
	if (found != NULL)
	{
		//item_t* found = g_aptr_crypto_page_cache_l1.get_internal(page_index);
                //ASSERT (found != NULL); // if NULL - abort! 

#ifdef APTR_RANDOM_ACCESS
		//if (__sync_val_compare_and_swap(&found->is_valid[sub_page_index], 0, 1) == 0)
		if (found->is_valid[sub_page_index] == 0)
		{
//			debug("major, found, and not yet valid for page %d sub %d ptr %p\n", page_index, sub_page_index, found);
			memcpy(*found->epc + sub_page_index * SUB_PAGE_SIZE, (uint8_t*)*prm_ptr + sub_page_index * SUB_PAGE_SIZE, SUB_PAGE_SIZE);
			found->is_valid[sub_page_index] = 100;
		}
//			debug("major, found, and was already valid for page %d sub %d ptr %p\n", page_index, sub_page_index, found);

		found->is_valid[sub_page_index]++; // = true;
		found->is_write[sub_page_index] |= aptr->hint_write;
#endif

		//item_t* found1 = g_aptr_crypto_page_cache_l1.get_internal(page_index);
		//debug("major, found, now page_index is %d sub %d valid is %d vs %d\n", page_index, sub_page_index, found1->is_valid[sub_page_index], found->is_valid[sub_page_index]);
		spin_lock(&g_free_epc_pages_lock);
	...
	//else {
	//	debug("major, first add for page %d sub %d\n", page_index, sub_page_index);
	//}
g_aptr_crypto_page_cache_l1.unlock(page_index);
g_aptr_crypto_page_cache_llc.unlock(page_index);
...
int aptr_fsync(aptr_t* aptr, int base_page_index)
{
	...
	g_aptr_crypto_page_cache_llc.lock(page_index);
	item_t* it = g_aptr_crypto_page_cache_l1.try_remove(page_index, aptr->sub_page_index);
	if (it == NULL)
	{
...
//	debug("removed page %d\n", page_index);
	...
	//if (aptr->hint_write)
	{
		int sub_page_index = i;// aptr->sub_page_index;i
		if (!it->is_write[sub_page_index])
		{
//			debug("no write for page_index=%d, i=%d\n", page_index, i);
			continue;
		}
		...
		unsigned char* epc_ptr = *it->epc + sub_page_index * SUB_PAGE_SIZE;
...
//debug("encrypted page_index=%d sub_page_index=%d\n", page_index, sub_page_index);
	...
	spin_lock(&g_free_epc_pages_lock);
	g_free_epc_pages.push_back(it->epc);
	...
	for ( ; n > 0; MOVE_APTR(&aptr.m_aptr, 1), s2++, --n)
    	{
	    	...
	    	if (*val != *s2)
	    	{
			debug("***********************FAILED with page index=%d sub_page_index=%d\n", aptr.m_aptr.page+aptr.m_base_page_index, aptr.m_aptr.sub_page_index);
